...
public class ActionIzmenaKorisnika extends AbstractAction {
    ...
    public String execute(HttpServletRequest request) {
        try {
            if (validacija(request)) {
                ...
                Korisnik k1 = new Korisnik(k.getKorisnikId(), ime, prezime, datumRodjenja, email, korisnickoIme, korisnickaSifra);
                DatabaseBroker.getInstance().izmeniKorisnika(k1);
    ...
    private boolean validacija(HttpServletRequest request) throws ParseException {
        ...
        String email = request.getParameter("email");
        String korisnickoIme = request.getParameter("korisnickoIme");
        String korisnickoImeStaro = ((Korisnik) request.getSession().getAttribute(WebConstants.USER_LOGIN)).getKorisnickoIme();
        String korisnickaSifra = request.getParameter("korisnickaSifra");
        ...
        if (korisnickoIme.equals("")) {
            ...
            imaGreska = true;
        } else if (!DatabaseBroker.getInstance().zauzetoKorisnickoIme(korisnickoIme) && !korisnickoIme.equals(korisnickoImeStaro)) {
